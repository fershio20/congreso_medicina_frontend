...
interface ConsejoDirectivoPageData {
    title: string;
    description: string;
    featured_image?: {
        url: string;
...
interface Props {
    comite: Comite[];
    consejoDirectivoPageData: ConsejoDirectivoPageData | null;
...
export const getStaticProps: GetStaticProps<Props> = async () => {
  ...
  try {
    ...
    // Fetch consejo directivo page data
    let consejoDirectivoPageData = null;
    try {
      const consejoDirectivoPageRes = await fetch(CONSEJO_DIRECTIVO_ENDPOINT, {
        headers: { Accept: "application/json" },
      });
      if (consejoDirectivoPageRes.ok) {
        const consejoDirectivoPageJson = await consejoDirectivoPageRes.json();
        consejoDirectivoPageData = consejoDirectivoPageJson.data?.attributes || consejoDirectivoPageJson.data || null;
      }
    } catch (error) {
      console.error("Error fetching consejo directivo page data:", error);
    }
...
export default function ComisionDirectivaPage({ comite, consejoDirectivoPageData, globalSEO, logoUrl }: Props) {
    // Determine the background image for PageHeader
    const backgroundImage = consejoDirectivoPageData?.featured_image?.formats?.large?.url 
        ? `${URL_DOMAIN}${consejoDirectivoPageData.featured_image.formats.large.url}`
        : consejoDirectivoPageData?.featured_image?.url 
        ? `${URL_DOMAIN}${consejoDirectivoPageData.featured_image.url}`
        : "/bg-default-blue.png";
    ...
    return (
        <>
                 ...
                 pageDescription={consejoDirectivoPageData?.description || "Conoce a los miembros de la Comisión Directiva de la Sociedad Paraguaya de Pediatría."}
                 pageImage={backgroundImage}
                 pageUrl={`${URL_DOMAIN}/autoridades/comision-directiva`}
            ...
            <div className="bg-white text-gray-800 space-y-12">
                ...
                <PageHeader 
                    title={consejoDirectivoPageData?.title || "Comisión Directiva SPP"}
                    description={consejoDirectivoPageData?.description || "Conoce a los miembros de la Comisión Directiva de la Sociedad Paraguaya de Pediatría."}
                    ...
                    <div className="container max-w-[1280px] mx-auto px-4">
                        {comite.length > 0 ? (
                                                ...
                                                <div key={comiteSection.id} className="mb-12">
                                                    <div className="flex flex-wrap justify-center gap-4">
                                                        {comiteSection.Miembros.map((miembro) => (
                                                            <div key={miembro.id} className="bg-white border-b border-[#0F62A5] p-6 text-center w-full sm:w-[calc(50%-0.5rem)] lg:w-[calc(25%-0.75rem)]" style={{ borderBottomColor: 'rgba(15, 98, 165, 0.41)' }}>
                            ...
                            <div className="text-center py-20">
                                ...
                                <h3 className="text-xl font-semibold text-gray-600 mb-2">No hay miembros disponibles</h3>
                                <p className="text-gray-500">La información de la Comisión Directiva se publicará próximamente.</p>
                            </div>
                        )}
                    </div>
                </section>
